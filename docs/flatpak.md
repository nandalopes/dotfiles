...
# flatpak pin --remove runtime/org.gimp.GIMP.Plugin.GMic/x86_64/2-40
```
...
sudo flatpak --installation=nuvola remote-add --if-not-exists nuvola \
  https://dl.tiliado.eu/flatpak/nuvola.flatpakrepo
sudo flatpak --installation=nuvola remote-add --if-not-exists flathub \
  https://flathub.org/repo/flathub.flatpakrepo
